...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let students = []; let currentStudentIndex = null; let currentDate = new Date(); let clickedDateStr = '';
    ...
    function loadData() { const savedData = localStorage.getItem('attendanceData'); students = savedData ? JSON.parse(savedData) : [{ name: "First Student", attendance: {} }]; }
    ...
    function renderStudentTabs() { studentWheel.innerHTML = ''; students.forEach((student, index) => { const tab = document.createElement('div'); tab.className = 'student-tab'; tab.textContent = student.name; const colorName = COLOR_NAMES[index % COLOR_NAMES.length]; const colorHex = STUDENT_COLORS[colorName]; tab.style.backgroundColor = `var(${colorName})`; if (isColorDark(colorHex)) { tab.style.color = 'var(--text-light)'; } else { tab.style.color = 'var(--text-dark)'; } tab.dataset.index = index; tab.addEventListener('click', handleStudentTabClick); studentWheel.appendChild(tab); }); setTimeout(updateWheelAnimation, 0); }
    addStudentBtn.addEventListener('click', () => { const newName = prompt("Enter the new student's name:"); if (newName && newName.trim()) { students.push({ name: newName.trim(), attendance: {} }); saveData(); renderStudentTabs(); } });
    deleteStudentBtn.addEventListener('click', () => { if (currentStudentIndex === null) return; const studentName = students[currentStudentIndex].name; if (confirm(`Are you sure you want to delete ${studentName}? This action cannot be undone.`)) { students.splice(currentStudentIndex, 1); saveData(); renderStudentTabs(); showPage(studentsPage); } });
